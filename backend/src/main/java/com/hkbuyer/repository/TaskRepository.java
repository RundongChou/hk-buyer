...
public class TaskRepository {
    ...
    public long createTask(Long orderId, BigDecimal suggestedMarkup, LocalDateTime acceptDeadline) {
        String sql = "INSERT INTO procurement_task(order_id, buyer_id, task_status, publish_at, accept_deadline, suggested_markup, accepted_at, updated_at) " +
    ...
    public Optional<ProcurementTask> findById(Long taskId) {
        String sql = "SELECT task_id, order_id, buyer_id, task_status, publish_at, accept_deadline, suggested_markup, accepted_at, updated_at " +
    ...
    public void acceptTask(Long taskId, Long buyerId) {
        String sql = "UPDATE procurement_task SET task_status = ?, buyer_id = ?, accepted_at = NOW(), updated_at = NOW() WHERE task_id = ?";
        jdbcTemplate.update(sql, TaskStatus.ACCEPTED.name(), buyerId, taskId);
    }
